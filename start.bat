...
for %%C in (py python python3) do (
    %%C -c "import sys; sys.exit(0 if sys.version_info >= (3, 10) else 1)" >/dev/null 2>&1
    if not errorlevel 1 (
        set PY=%%C
        goto :found
    )
...
fc /b requirements.txt "%STAMP%" >/dev/null 2>&1
...
copy /y requirements.txt "%STAMP%" >/dev/null 2>&1
